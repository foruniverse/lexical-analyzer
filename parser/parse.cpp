...
typedef enum 
   ...
   {ENDFILE,ERROR,
    /* reserved words */
    IF,THEN,ELSE,END,REPEAT,UNTIL,READ,WRITE,BEGIN,FOR,WHILE,CONTINUE,BREAK,
...
static struct
   ...
   = {{"if",IF},{"then",THEN},{"else",ELSE},{"end",END},
      ...
      {"write",WRITE} ,{"while",WHILE},{"for",FOR},{"break",BREAK},{"continue",CONTINUE}};
...
{ if (token == expected) token = getToken();
    ...
    char p[]="unexpected token -> ";
    syntaxError(p);
    printToken(token,tokenString);
    // fprintf(listing ,", this token may be ");
    //cout<<expected<<endl;
...
int main()
{
    ...
    if(source ==NULL)
    {
        fprintf(listing,"file open failed\n");
        exit(1);
    }
    syntaxTree = parse();
